...
async function dataGetter(mei: string ) {
...
export async function getData(mei: string) {
  ...
  const topLevelData = await dataGetter(mei)
  const cardResponse = await Promise.all(
    topLevelData.APIResponse.Content.map((item) => dataGetter(item.MediaEncryptedIdentifier))
  )
